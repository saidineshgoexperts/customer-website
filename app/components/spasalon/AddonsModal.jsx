        ...
        try {
            ...
            if (data.success && data.data && data.data.length > 0) {
                ...
                if (allAddons.length > 0) {
                    ...
                    // No addons found, skip modal
                    console.log("⚠️ No add-ons found in data, skipping...");
                    setAddons([]);
                    onContinue([]);
                }
            } else {
                console.log("⚠️ No add-ons data returned, skipping...");
                setAddons([]);
                onContinue([]);
            }
        } catch (err) {
            ...
            {isOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
                    ...
                    <motion.div
                        ...
                        className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh] border border-[#C06C84]/20"
                    >
                        {/* Header */}
                        <div className="p-6 border-b border-[#C06C84]/10 flex items-center justify-between bg-gradient-to-r from-pink-50 to-white">
                            <div>
                                <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                                    <Sparkles className="w-6 h-6 text-[#C06C84]" />
                                    Enhance Your Experience
                                </h2>
                                <p className="text-gray-500 text-sm mt-1">Add extra treatments or essentials</p>
                            </div>
                            <button
                                onClick={onClose}
                                className="p-2 rounded-full hover:bg-pink-100 transition-colors"
                            >
                                <X className="w-6 h-6 text-gray-500 hover:text-[#C06C84]" />
                            </button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 scrollbar-hide">
                            {loading ? (
                                ...
                                <div className="text-center py-12 text-gray-500 bg-gray-50 rounded-2xl flex flex-col items-center border border-dashed border-gray-300">
                                    <Flower className="w-12 h-12 text-gray-300 mb-4" />
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`relative p-4 rounded-xl border-2 cursor-pointer transition-all duration-200 group ${isSelected
                                                    ? 'border-[#C06C84] bg-[#C06C84]/5 shadow-sm'
                                                    : 'border-gray-100 hover:border-[#C06C84]/30 hover:shadow-md'
                                                    }`}
                                            >
                                                <div className="flex items-start gap-4">
                                                    {/* Image or Icon */}
                                                    <div className={`w-16 h-16 rounded-lg overflow-hidden flex-shrink-0 ${isSelected ? 'ring-2 ring-[#C06C84] ring-offset-2' : 'bg-gray-100'}`}>
                                                        {addon.image ? (
                                                            <img
                                                            ...
                                                            <div className="w-full h-full flex items-center justify-center bg-pink-50 text-[#C06C84]/40">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex items-start justify-between">
                                                            <h3 className={`font-semibold text-sm truncate pr-2 ${isSelected ? 'text-[#C06C84]' : 'text-gray-900'}`}>
                                                                {addon.childServiceName}
                                                            </h3>
                                                            {isSelected && (
                                                                <div className="w-5 h-5 bg-[#C06C84] rounded-full flex items-center justify-center shrink-0">
                        ...
                        <div className="p-6 border-t border-gray-100 bg-gray-50 flex items-center justify-between">
                            ...
                            <div className="flex gap-3">
                                ...
                                <button
                                    ...
                                    className="px-8 py-2.5 rounded-xl bg-gradient-to-r from-[#C06C84] to-[#6C5CE7] text-white font-bold shadow-lg hover:shadow-[#C06C84]/20 transition-all transform active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed"
                                >
