      ...
      try {
        ...
        if (data.success && data.data) {
          ...
          const transformedArticles = data.data.slice(0, 4).map(article => {
            ...
            // If using default image, try to use subcategory, category, or service image as fallback
            if (imageUrl.includes('default_image.jpg')) {
              if (article.subcategoryId?.image) {
                imageUrl = article.subcategoryId.image;
              } else if (article.categoryId?.image) {
                imageUrl = article.categoryId.image;
              } else if (article.serviceId?.image) {
                imageUrl = article.serviceId.image;
              }
            }
